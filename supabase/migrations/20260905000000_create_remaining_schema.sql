-- =========================================================
-- Create the rest of the schema on top of the `profiles` table,
-- which is confirmed to already exist live in Supabase (created
-- manually via the Studio SQL editor before this migration folder
-- existed). Everything else below is new.
--
-- Written directly in its final, hardened form (the fixes that
-- would otherwise be separate Task 1/2/3 migrations — cross-passage
-- question_number uniqueness, RLS gating on submitted attempts,
-- publish-validation trigger) since these tables have no existing
-- data to migrate/backfill.
--
-- Guards (`do $$ ... exception when duplicate_object`, `create or
-- replace`, `drop ... if exists`) are used only where something
-- might already exist alongside the bare `profiles` table (the
-- `user_role` enum it depends on, and its own trigger/policies).
-- Everything else is a plain `create`, since those objects cannot
-- exist yet without their parent table.
-- =========================================================

-- ---------- ENUMS ----------
-- user_role almost certainly already exists (profiles.role depends on it).
do $$ begin
...
-- ---------- PROFILES (table already exists — just ensure the rest) ----------
